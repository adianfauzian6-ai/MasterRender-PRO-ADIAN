...
APP_NAME='MasterRender PRO by AfroxSTD (ADIAN)'; MAX_VIDEOS=60; MAX_SONGS=20
AUDIO_EXT={'.mp3','.wav','.m4a','.aac','.flac','.ogg','.opus'}; BG='#0b0b12'; PANEL='#12121c'; INPUT='#0d0d15'; PURPLE='#8b5cf6'; TEXT='#eceaf5'; MUTED='#858198'
def root_dir(): return Path(getattr(sys,'_MEIPASS',Path(sys.executable).parent)) if getattr(sys,'frozen',False) else Path(__file__).resolve().parent
def ffmpeg_path():
 p=root_dir()/'ffmpeg.exe'; return str(p) if p.exists() else (shutil.which('ffmpeg') or '')
def hidden_kwargs():
...
class App(tk.Tk):
 def __init__(self):
  ...
  self.transition=tk.StringVar(value='Memudar (Fade)'); self.engine=tk.StringVar(value='Otomatis Maksimal'); self.quality=tk.StringVar(value='Mengikuti Master'); self.output_size=tk.StringVar(value='Mengikuti Master'); self.variations=tk.IntVar(value=1); self.song_count=tk.IntVar(value=9); self.album_repeat=tk.IntVar(value=1); self.shuffle=tk.BooleanVar(value=True); self.normalize=tk.BooleanVar(value=False); self.timestamp=tk.BooleanVar(value=False); self.make_variations=tk.BooleanVar(value=False); self.output_dir=tk.StringVar(value=str(Path.cwd()/'output')); self.output_name=tk.StringVar(value='masterrender')
 ...
 def log(self,tag,msg):
  if hasattr(self,'logbox'): self.after(0,lambda:self._log(tag,msg))
 ...
 def start_render(self):
  ...
  if not ffmpeg_path(): return messagebox.showerror('FFmpeg tidak ditemukan','ffmpeg.exe tidak ditemukan di folder aplikasi.')
  ...
  try:n=max(1,min(20,int(self.song_count.get()))); reps=max(1,min(20,int(self.album_repeat.get()))); vars_=max(1,min(100,int(self.variations.get()))) if self.make_variations.get() else 1
  except Exception:return messagebox.showerror('Pengaturan salah','Nilai pengaturan tidak valid.')
 ...
 def choose_playlist(self,n,reps):
  chosen=[]
  for _ in range(reps):
   block=list(self.songs); random.shuffle(block) if self.shuffle.get() else None; chosen.extend(block[:n])
  return chosen
 def encoder(self):
 ...
 def render_one(self,video,playlist,outfile,video_no,total_videos):
  ...
  filt=f'[0:v]{self.output_filter()}[vout];'+';'.join(parts); enc,name=self.encoder(); args+=['-filter_complex',filt,'-map','[vout]','-map',amap]+enc+['-c:a','aac','-b:a','192k','-shortest','-movflags','+faststart',str(outfile)]
  self.log('RENDER',f'Video {video_no}/{total_videos} • {n} lagu • Encoder: {name}'); self.log('PLAYLIST',' → '.join(Path(x).name for x in playlist))
  self.proc=subprocess.Popen(args,stdout=subprocess.DEVNULL,stderr=subprocess.PIPE,text=True,encoding='utf-8',errors='replace',**hidden_kwargs()); duration=0.0; duration_seen=0; last=[]; start=time.time(); last_pct=-1
  while True:
   ...
   if line:
    ...
    if m and duration>0:
     ...
     if pct>=last_pct+0.5:last_pct=pct; self.after(0,lambda p=pct,v=video_no,t=total_videos:self.update_progress(p,v,t))
  ...
  if rc!=0:
   ...
   for x in last[-10:]:self.log('FFMPEG',x)
   try:Path(outfile).unlink(missing_ok=True)
   except Exception:pass
   return False
  self.after(0,lambda v=video_no,t=total_videos:self.update_progress(100,v,t)); return True
 def update_progress(self,pct,vi,total):
  overall=(vi-1)*100/total+pct/total; self.progress['value']=overall; self.progress_text.config(text=f'Video {vi}/{total} • {pct:.1f}% • Total {overall:.1f}%')
