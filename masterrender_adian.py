...
def get_ffmpeg():
    roots=[Path(getattr(sys,'_MEIPASS',''))/'' ,Path(sys.executable).parent] if getattr(sys,'frozen',False) else [Path(__file__).parent]
    for r in roots:
        p=r/'ffmpeg.exe'
...
class App(tk.Tk):
    ...
    def section_title(self,p,text,size=11): tk.Label(p,text=text,bg=p.cget('bg'),fg='#eef0ff',font=('Segoe UI',size,'bold')).pack(anchor='w',padx=18,pady=(18,8))
    def make_ui(self):
    ...
    def render(self,video,songs,vi,k):
        out=self.unique(Path(self.out.get())/f'{Path(video).stem}_MasterRender_V{k}.mp4'); preset='veryfast' if self.quality.get()=='Cepat' else 'slow' if self.quality.get()=='Kualitas Tinggi' else 'medium'; crf='23' if preset=='veryfast' else '18' if preset=='slow' else '20'
        cmd=[self.ff,'-hide_banner','-nostdin','-y','-stream_loop','-1','-i',video]; parts=[]
        ...
        parts.append(''.join(f'[a{i}]' for i in range(1,len(songs)+1))+f'concat=n={len(songs)}:v=0:a=1[playlist]'); cmd += ['-filter_complex',';'.join(parts),'-map','0:v:0','-map','[playlist]','-vf','scale=trunc(iw/2)*2:trunc(ih/2)*2,format=yuv420p','-c:v','libx264','-preset',preset,'-crf',crf,'-c:a','aac','-b:a','192k','-shortest','-movflags','+faststart',str(out)]; return self.run(cmd,out)
    def run(self,cmd,out):
        try:
            ...
            for line in self.proc.stdout:
                line=line.rstrip(); tail.append(line); tail=tail[-12:]; low=line.lower()
